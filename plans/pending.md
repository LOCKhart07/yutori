...
- **Tx-list sort-by-amount is buggy + wrong default.** On the drill-down screens the amount-sort produces the wrong order (reported by user 2026-04-15, code not yet investigated — could be sign/refund handling, null `inrAmount` coercion to `0.0`, or comparator direction). Two things in one fix: (1) make sort-by-amount correct, (2) change the default from `DateDesc` to `AmountDesc` (largest first) so the user lands on the most impactful txs immediately. The `TxSort` cycle order should likely lead with `AmountDesc` too.
