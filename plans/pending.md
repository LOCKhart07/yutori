...
- **Historical import doesn't update the dashboard incrementally.** Original intent (per spec) was "rows show up as they're parsed" so the user can watch progress. Today the dashboard appears empty/stale until the worker finishes the entire batch. Likely causes to verify: (a) `HistoricalImportWorker` wraps all inserts in a single Room transaction, so the `Flow` emits nothing until commit; (b) imports land in *past* months so the current-month dashboard naturally doesn't change — the visual feedback should be on the import progress block + a month-navigator badge or "view March" affordance; (c) the dashboard's `WhileSubscribed(5s)` window swallows the burst of emissions if the worker out-paces the UI subscription. Investigate, then fix whichever combination is real.
...
- **Account drill-down from dashboard footer shows ₹0 spend.** Tapping an account row in the accounts strip at the bottom of the dashboard opens the drill-down with zero total and no transactions, even when that account has activity this month. Reported 2026-04-15, not yet investigated. Likely suspects: (a) the drill-down query filters on `account_id` but dashboard rows are keyed by something else (last4? issuer?) so the navigation arg doesn't match any tx rows; (b) tx rows have `account_id = null` for UPI-handle-matched accounts, so an account-id filter finds nothing; (c) the month-key filter on the drill-down uses the wrong month (default "current" instead of the dashboard's navigated month). Check the nav arg + query before picking a fix.
