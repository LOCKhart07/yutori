...
class YutoriSmsReceiver : BroadcastReceiver() {
    ...
    companion object {
        private const val TAG = "YutoriSmsReceiver"
    }
}

/**
 * Holder for the app-wide [IngestionPipeline] so the receiver can find
 * it without dependency injection. The [YutoriApp] sets
...
class IngestionCoordinator(
     ...
     * [com.yutori.forex.ForexConversionWorker] so new pending-forex
    ...
    private fun notifyTransactionIngestedIfNeeded(outcome: IngestionOutcome) {
        ...
        val isTxEvent = decision is com.yutori.transactions.MergeDecision.CreatedNew ||
            decision is com.yutori.transactions.MergeDecision.MergedInto
        if (isTxEvent) {
            try {
    ...
    companion object {
        ...
        /** Global instance — set by [YutoriApp.onCreate]. */
        @Volatile
