...
./gradlew :parser:test --tests "com.spendwise.parser.rules.KotakRuleTest"
...
# Release APK (debug-signed unless SPENDWISE_* env vars set — see docs/RELEASING.md)
...
- **`:database`** (Android library + KSP Room) — Room entities, DAOs, mappers, `SpendWiseDatabase`. Schemas exported to `android/database/schemas/` and checked in — Room migrations diff against them.
...
Package layout in each module is `com.spendwise.<module>`; `:app` uses `com.spendwise.{ui,notifications,forex,importing,backup}`.
